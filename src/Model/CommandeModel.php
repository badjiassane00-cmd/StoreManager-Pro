<?php

require_once dirname(__DIR__) . "/Core/Database.php";
require_once __DIR__ . "/Entity/Commande.php";
require_once __DIR__ . "/Entity/LigneCommande.php";
require_once __DIR__ . "/ClientModel.php";
require_once __DIR__ . "/UtilisateurModel.php";
require_once __DIR__ . "/ProduitModel.php";

class CommandeModel
{
    private static ?Database $db = null;

    private static function getDb(): Database
    {
        if (self::$db === null) {
            self::$db = Database::getInstance();
        }

        return self::$db;
    }

    private static function convertirEnCommande(array $ligne): Commande
    {
        $client = $ligne["client_id"] !== null
            ? ClientModel::getClientParId((int) $ligne["client_id"])
            : null;

        $utilisateur = UtilisateurModel::getUtilisateurParId(
            (int) $ligne["utilisateur_id"]
        );

        return new Commande(
            (int) $ligne["id"],
            $client,
            $utilisateur,
            (float) $ligne["montant_verse"],
            $ligne["date_vente"]
        );
    }

    private static function convertirEnLigneCommande(
        array $ligne,
        Commande $commande
    ): LigneCommande {
        
        $produit = ProduitModel::getProduitParId(
            (int) $ligne["produit_id"]
        );

        return new LigneCommande(
            (int) $ligne["id"],
            $commande,
            $produit,
            (int) $ligne["quantite"],
            (float) $ligne["prix_unitaire"]
        );
    }

    public static function creerCommande(Commande $commande): int
    {
        $sql = "INSERT INTO commandes
                (date_vente, client_id, utilisateur_id, montant_total, montant_verse, statut_paiement)
                VALUES
                (:date_vente, :client_id, :utilisateur_id, :montant_total, :montant_verse, :statut_paiement)";

        return self::getDb()->executeUpdate($sql, [
            "date_vente" => $commande->getDateVente(),
            "client_id" => $commande->getClient()?->getId(),
            "utilisateur_id" => $commande->getUtilisateur()->getId(),
            "montant_total" => $commande->getMontantTotal(),
            "montant_verse" => $commande->getMontantVerse(),
            "statut_paiement" => $commande->getStatutPaiement()
        ]);
    }

    public static function ajouterLigne(
        int $commandeId,
        LigneCommande $ligne
    ): int {
        
        $sql = "INSERT INTO lignes_commandes
                (commande_id, produit_id, quantite, prix_unitaire)
                VALUES
                (:commande_id, :produit_id, :quantite, :prix_unitaire)";

        return self::getDb()->executeUpdate($sql, [
            "commande_id" => $commandeId,
            "produit_id" => $ligne->getProduit()->getId(),
            "quantite" => $ligne->getQuantite(),
            "prix_unitaire" => $ligne->getPrixUnitaire()
        ]);
    }

    public static function getLignesParCommande(
        Commande $commande
    ): array {

        $sql = "SELECT * FROM lignes_commandes
                WHERE commande_id = :commande_id";

        $lignesSql = self::getDb()->executeQuery($sql, [
            "commande_id" => $commande->getId()
        ], false);

        $lignes = [];

        foreach ($lignesSql as $ligneSql) {
            $lignes[] = self::convertirEnLigneCommande(
                $ligneSql,
                $commande
            );
        }

        return $lignes;
    }

    public static function getCommandeParId(int $id): ?Commande
    {
        $sql = "SELECT * FROM commandes WHERE id = :id";

        $ligne = self::getDb()->executeQuery($sql, [
            "id" => $id
        ]);

        if (!$ligne) {
            return null;
        }

        $commande = self::convertirEnCommande($ligne);

        $lignes = self::getLignesParCommande($commande);

        foreach ($lignes as $ligneCommande) {
            $commande->ajouterLigne($ligneCommande);
        }

        return $commande;
    }

    public static function getCommandes(): array
    {
        $sql = "SELECT * FROM commandes ORDER BY id DESC";

        $lignes = self::getDb()->query($sql, false);

        $commandes = [];

        foreach ($lignes as $ligne) {
            $commandes[] = self::convertirEnCommande($ligne);
        }

        return $commandes;
    }
}